...
/* ════════════════════════════════════════════════════════
   Aura Weather — Major Storms
   Scans 76 major world cities simultaneously, scores each
   by storm severity, selects top 4 at least 250 mi apart.
   Always shows 4 results even in globally calm conditions.
   ════════════════════════════════════════════════════════ */
...
/* ── Storm severity ─────────────────────────────────── */
const CODE_SCORE = {
  99:12, 96:10, 95:9, 82:8, 86:7, 67:7,
  65:6,  75:6,  66:5, 81:5, 85:5, 73:4,
  63:4,  71:3,  77:3, 80:3, 61:2, 55:2,
  53:1,  51:1,
};

function stormScore(c) {
  let s = CODE_SCORE[c.weather_code] || 0;
  const w = c.wind_speed_10m || 0;
  if (w > 80) s += 5; else if (w > 55) s += 3; else if (w > 35) s += 2; else if (w > 20) s += 1;
  const p = c.precipitation || 0;
  if (p > 20) s += 4; else if (p > 10) s += 3; else if (p > 3) s += 2; else if (p > 0.5) s += 1;
  return s;
}
...
function stormLabel(code, wind) {
  ...
  if ((wind || 0) * 0.621371 > 50)        return 'High Winds';
...
function severityLevel(score) {
  if (score >= 12) return { label:'Extreme',  color:'#F44336', bars:5 };
  if (score >= 8)  return { label:'Severe',   color:'#FF5722', bars:4 };
  if (score >= 5)  return { label:'Heavy',    color:'#FF9800', bars:3 };
  if (score >= 2)  return { label:'Moderate', color:'#FDD835', bars:2 };
  return               { label:'Active',   color:'#81C784', bars:1 };
}
...
function selectSpread(ranked, n = 4, minMi = 250) {
  const sel = [];
  for (const city of ranked) {
    if (sel.length >= n) break;
    if (!sel.some(s => haversineMiles(s.lat, s.lon, city.lat, city.lon) < minMi))
      sel.push(city);
  }
  // If we couldn't find 4 spread enough, relax distance and fill up
  if (sel.length < n) {
    for (const city of ranked) {
      if (sel.length >= n) break;
      if (!sel.includes(city)) sel.push(city);
    }
  }
  return sel;
}
...
async function findActiveStorms() {
  ...
  const fetchCity = (city) => {
    ...
    return fetch(`https://api.open-meteo.com/v1/forecast?${params}`, { signal: controller.signal })
      .then(r => { clearTimeout(timer); return r.ok ? r.json() : null; })
      .then(data => (data && data.current) ? { ...city, current: data.current } : null)
      .catch(() => null);
  };

  // Fire ALL requests simultaneously — browser queues them efficiently
  const allResults = await Promise.all(WORLD_CITIES.map(fetchCity));
  const valid = allResults.filter(Boolean);

  if (valid.length === 0) throw new Error('All city fetches failed');

  const ranked = valid
    .map(c => ({ ...c, score: stormScore(c.current) }))
    .sort((a, b) => b.score - a.score);

  return selectSpread(ranked, 4, 250);
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('storms-refresh').addEventListener('click', async () => {
    const btn = document.getElementById('storms-refresh');
    btn.disabled = true;
    await loadStorms();
    btn.disabled = false;
  });
...
async function loadStorms() {
  ...
  try {
    ...
    console.error('Storm detection error:', err);
    document.getElementById('storms-grid').innerHTML =
      `<p style="color:rgba(255,255,255,0.50);padding:24px;grid-column:span 2">
        Could not reach weather servers. Check your connection and try refreshing.
      </p>`;
  } finally {
...
function renderStorms(storms) {
  ...
  if (!storms || storms.length === 0) {
    grid.innerHTML = `<p style="color:rgba(255,255,255,0.50);padding:24px;grid-column:span 2">
      No significant storm activity detected globally right now.
    </p>`;
    return;
  }
  ...
  storms.forEach((storm, i) => {
    ...
    const wind  = Math.round((c.wind_speed_10m  || 0) * 0.621371);
    const gusts = Math.round((c.wind_gusts_10m  || 0) * 0.621371);
    const temp  = Math.round((c.temperature_2m  || 0) * 9/5 + 32);
    const dir   = degreesToCardinal(c.wind_direction_10m || 0);
    const precip = (c.precipitation || 0).toFixed(1);
    const hum   = Math.round(c.relative_humidity_2m || 0);
    const score = storm.score;
    ...
    card.innerHTML = `
      <div class="storm-rank">#${i + 1} globally</div>
      ...
      <div class="storm-icon-row">
        ${stormSVG(c.weather_code, sev.color)}
      ...
      <div class="storm-metrics">
        <div class="storm-metric">
          <svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M3,7 Q8,3 12,7 Q16,11 12,14"/><path d="M1,11 L14,11"/></svg>
          <div class="sm-value">${wind}<span class="sm-unit"> mph</span></div>
          <div class="sm-label">Wind ${dir}</div>
        </div>
        <div class="storm-metric">
          <svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M1,9 L14,9"/><path d="M1,13 L10,13"/><path d="M3,5 Q9,1 13,5"/></svg>
          <div class="sm-value">${gusts}<span class="sm-unit"> mph</span></div>
          <div class="sm-label">Gusts</div>
        </div>
        <div class="storm-metric">
          <svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M4,10 Q4,5 10,4 Q16,5 16,10 Q18,14 14,15 L6,15 Q2,14 4,10Z"/><line x1="8" y1="17" x2="7" y2="20"/><line x1="12" y1="17" x2="11" y2="20"/></svg>
          <div class="sm-value">${precip}<span class="sm-unit"> mm</span></div>
          <div class="sm-label">Precip</div>
        </div>
        <div class="storm-metric">
          <svg viewBox="0 0 20 20" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M10,2 Q15,8 15,13 A5,5 0 0 1 5,13 Q5,8 10,2Z"/></svg>
          <div class="sm-value">${hum}<span class="sm-unit">%</span></div>
          <div class="sm-label">Humidity</div>
        </div>
      </div>

      <div class="storm-severity-bar">
        ${[1,2,3,4,5].map(n => `<div class="severity-seg${n <= sev.bars ? ' active' : ''}"
          style="${n <= sev.bars ? 'background:' + sev.color : ''}"></div>`).join('')}
      </div>
      <div class="storm-score-row">
        <span class="storm-coords">${Math.abs(storm.lat).toFixed(1)}°${storm.lat>=0?'N':'S'} &middot; ${Math.abs(storm.lon).toFixed(1)}°${storm.lon>=0?'E':'W'}</span>
        <span class="storm-score-val">Severity score: ${score}</span>
      </div>`;
...
/* ── Storm weather SVG ──────────────────────────────── */
function stormSVG(code, color) {
  const cloud = `<path d="M12,34 Q8,26 14,18 Q20,8 32,10 Q36,4 44,6 Q56,8 56,20 Q62,24 60,32 Q58,40 48,40 L18,40 Q10,40 12,34Z" stroke="${color}" stroke-width="2" fill="${color}18" stroke-linejoin="round"/>`;
  if ([95,96,99].includes(code)) return `<svg viewBox="0 0 64 64" width="62" height="62" fill="none"><path d="M12,36 Q8,28 14,20 Q20,10 32,12 Q36,6 44,8 Q56,10 56,22 Q62,26 60,34 Q58,42 48,42 L18,42 Q10,42 12,36Z" stroke="${color}" stroke-width="2" fill="${color}18" stroke-linejoin="round"/><path d="M34,42 L28,54 L36,52 L30,64" stroke="${color}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>`;
  if ([65,63,80,81,82].includes(code)) return `<svg viewBox="0 0 64 64" width="62" height="62" fill="none">${cloud}<line x1="22" y1="44" x2="18" y2="58" stroke="${color}" stroke-width="2" stroke-linecap="round"/><line x1="32" y1="44" x2="28" y2="58" stroke="${color}" stroke-width="2" stroke-linecap="round"/><line x1="42" y1="44" x2="38" y2="58" stroke="${color}" stroke-width="2" stroke-linecap="round"/></svg>`;
  if ([71,73,75,77,85,86].includes(code)) return `<svg viewBox="0 0 64 64" width="62" height="62" fill="none">${cloud}<circle cx="22" cy="50" r="3" fill="${color}" opacity="0.7"/><circle cx="32" cy="54" r="3" fill="${color}" opacity="0.7"/><circle cx="42" cy="50" r="3" fill="${color}" opacity="0.7"/></svg>`;
  return `<svg viewBox="0 0 64 64" width="62" height="62" fill="none"><path d="M14,36 Q10,28 16,20 Q22,10 34,12 Q38,6 46,8 Q58,10 58,22 Q64,26 62,34 Q60,42 50,42 L20,42 Q12,42 14,36Z" stroke="${color}" stroke-width="2" fill="${color}18" stroke-linejoin="round"/></svg>`;
}
